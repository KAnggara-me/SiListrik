...
@section('container')
  <section>
            ...
            <tbody>
              @foreach ($sensors as $s)
                ...
                <tr class="border-b border-gray-200 text-center text-xs text-black md:text-sm">
                  <td class="bg-grey-100 border-r border-gray-200 p-2">{{ $s->updated_at->format('d/m/y H:i:s') }}</td>

                  <td class="@php if ($s->voltase > 239) { echo 'bg-red-300'; } elseif ($s->voltase < 209) { echo 'bg-blue-300'; } else { echo 'bg-grey-100'; } @endphp border-r border-gray-200 p-2">{{ number_format($s->voltase) . ' V' }}</td>

                  <td class="{{ $s->arus > 4 ? 'bg-red-300' : 'bg-gray-100 ' }} border-r border-gray-200 p-2">{{ number_format($s->arus, $s->arus > 1 ? '1' : '2', '.', ',') . ' A' }}</td>

                  <td class="{{ $P > 880 ? 'bg-red-300' : 'bg-gray-100 ' }} border-r border-gray-200 p-2">{{ number_format($P, '0', '.', ',') . ' VA' }}</td>

                  <td class="{{ $s->temperatur > 50 ? 'bg-red-300' : 'bg-gray-100 ' }} border-r border-gray-200 p-2">{{ number_format($s->temperatur) }} C&deg;</td>

                  <td class="{{ $s->api > 0 ? 'bg-red-300' : 'bg-gray-100 ' }} border-r border-gray-200 p-2">{{ $s->api }}</td>

                  <td class="{{ $s->asap > 300 ? 'bg-red-300' : 'bg-gray-100 ' }} border-r border-gray-200 p-2">{{ number_format($s->asap) }}</td>
    ...
    <script>
      let a = 2;
      (a > 4 ? console.log('Labih dari 4') : a < 1) ? console.log("Kirang dari 1"): console.log("antara 1 dan 4");
    </script>
